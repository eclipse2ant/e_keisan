...
\
'一価飽和脂肪酸','多価不飽和脂肪酸','コルステロール','炭水化物','利用可能炭水化物','低分子量水溶性食物繊維' ,'高分子量水溶性食物繊維' ,'不溶性食物繊維','食物繊維総量','灰分','ナトリウム', \
\
...
class View
	...
	def to_s
		...
		@r_list.each do |recipe|
			...
			print "(塩分;#{recipe.v_seibun_per_person[53].roundoff(1)}g)" 
			print "\n" 
...
#		print "(DM交換表; #{recipe.v_seibun_per_person.roundoff(1).to_a[63..69].join(' ')})" 
		print "(DM交換表; #{recipe.v_seibun_per_person.roundoff(1).to_a[64..70].join(' ')})" 
			print "\n\n"
		end
	...
	puts ""
		...
		print "(塩分;#{@r_list.v_seibun_per_person[53].roundoff(1)}g)"
		print "\n" 
...
#		print "(DM交換表; #{@r_list.v_seibun_per_person.roundoff(1).to_a[63..69].join(' ')})" 
		print "(DM交換表; #{@r_list.v_seibun_per_person.roundoff(1).to_a[64..70].join(' ')})" 
		print "\n\n"
